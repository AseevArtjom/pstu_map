...
export const fetchBuildingGraph = createAsyncThunk<BuildingGraphResponse, string>(
    'map/fetchGraph',
    async (buildingId) => {
        const response = await http.get<BuildingGraphResponse>(`/map/${buildingId}`);
...
export const calculateRoute = createAsyncThunk<PathResponse, { fromRoomId: string; toRoomId: string }>(
    ...
    async ({ fromRoomId, toRoomId }) => {
        const response = await http.get<PathResponse>('/map/path', {
